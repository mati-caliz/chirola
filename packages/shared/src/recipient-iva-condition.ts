...
export function allowedRecipientIvaConditions(voucherType: number): readonly number[] {
  ...
  if (letter === LETTER_B) {
    return knownConditions.filter((condition) => condition !== RecipientIvaCondition.RESPONSABLE_INSCRIPTO);
  }
  return knownConditions;
}

export function isRecipientIvaConditionAllowed(voucherType: number, conditionId: number): boolean {

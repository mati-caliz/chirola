...
export type IssuerOnboardingStatusName =
  (typeof IssuerOnboardingStatus)[keyof typeof IssuerOnboardingStatus];

export const issuerOnboardingStatusName: Record<
...
export function describeIssuerOnboardingStatus(status: string): string {
  const known = Object.entries(issuerOnboardingStatusName).find(
    ([key]) => key === status,
  );

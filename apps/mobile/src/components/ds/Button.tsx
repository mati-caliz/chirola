...
function variantColors(theme: Theme, variant: ButtonVariant): { bg: string; press: string; fg: string } {
  const { colors } = theme;
  switch (variant) {
    case "secondary":
      return {
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        ...
        {
          height: text.height,
          paddingHorizontal: text.paddingHorizontal,
    ...
    >
      {loading ? (
        ...
        <>
          {icon}
          <Text style={{ color: fg, fontFamily: theme.font.semibold, fontSize: text.fontSize }}>
            {children}
          </Text>
        </>
      )}

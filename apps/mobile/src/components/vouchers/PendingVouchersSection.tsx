...
const PendingVoucherCard = ({
  ...
  const retry = useMutation({ mutationFn: () => retryPendingVoucher(issuerId, pending.id), onSuccess: refresh });
  const discard = useMutation({ mutationFn: () => discardPendingVoucher(issuerId, pending.id), onSuccess: refresh });
    ...
    <Card>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}>
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <Text style={{ marginTop: 2, fontFamily: theme.font.regular, fontSize: theme.fontSize.caption, color: theme.colors.textSecondary }}>
            {voucherTypeName[pending.voucherType] ?? `Tipo ${pending.voucherType}`} · {formatDate(pending.createdAt)}
          </Text>
        </View>
        <View style={{ alignItems: 'flex-end', gap: 6 }}>
          <Amount value={formatCurrency(pending.totalAmount, pending.currency)} size="sm" />
          <StatusBadge status={failed ? 'rechazado' : 'pendiente'} label={failed ? 'Falló' : 'En cola'} size="sm" />
        </View>
      </View>

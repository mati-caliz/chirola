...
import { emparejarCert, generarCsr } from '@/lib/resources';
...
export default function CertificadoScreen() {
  ...
  const csrMutation = useMutation({
    mutationFn: () => generarCsr(emisorId, alias.trim() || undefined),
    onSuccess: (res) => {
      setError(null);
  ...
  const certMutation = useMutation({
    mutationFn: () => emparejarCert(emisorId, certPem.trim()),
    onSuccess: () => {
      ...
      <Screen>
        ...
        {csrPem ? (
          <Card>
            ...
            />
            <Button
              title={copiado ? '¡Copiado!' : 'Copiar CSR'}
              variant="secondary"
              onPress={copiarCsr}
            />
          </Card>

...
describe("PadronService", () => {
  ...
  it("deduce responsable inscripto cuando está inscripto en IVA", async () => {
    ...
    expect(taxpayer.ivaConditionId).toBe(RecipientIvaCondition.RESPONSABLE_INSCRIPTO);
  });
  ...
  it("traduce el fault de CUIT inexistente en un 404", async () => {
    ...
    await expect(service().getTaxpayer(auth, "20999999999")).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it("propaga los demás errores de ARCA sin convertirlos en 404", async () => {
    ...
    await expect(service().getTaxpayer(auth, "20111111112")).rejects.not.toBeInstanceOf(NotFoundException);
  });

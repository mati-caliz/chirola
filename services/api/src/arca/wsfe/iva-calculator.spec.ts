...
describe("calculateAmounts", () => {
  ...
  it("Factura A: discrimina IVA 21% desde el bruto y cuadra", () => {
    const r = calculateAmounts(VoucherType.FACTURA_A, [taxed({ description: "Servicio" })]);
  ...
  it("Factura B: informa neto e IVA a ARCA aunque no los discrimine al cliente", () => {
    const r = calculateAmounts(VoucherType.FACTURA_B, [taxed({ description: "Servicio" })]);
...
describe("calculateAmounts — exentos y no gravados", () => {
  ...
  it("mantiene la identidad de totales que valida ARCA", () => {
    ...
    expect(r.untaxedAmount + r.netAmount + r.exemptAmount + r.ivaAmount + r.tributeAmount).toBeCloseTo(
      r.totalAmount,
      2,
    );
  });
...
describe("calculateAmounts — tributos", () => {
  ...
  it("aplica tributos también en Factura C", () => {
    const r = calculateAmounts(VoucherType.FACTURA_C, [taxed({ unitPrice: 1000 })], [perception]);

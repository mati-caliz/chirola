...
import {
  ...
  type IssuerOnboardingStatusName,
} from '@chirola/shared';
import { PrismaService } from '../prisma/prisma.service';
import {
  ArcaDelegationError,
  ArcaRejectionError,
  ARCA_TOKEN_RELATION_CODE,
...
export type ArcaConfirmationKind =
  (typeof ArcaConfirmation)[keyof typeof ArcaConfirmation];

const CONFIRMED_STATUS: Record<
  ArcaConfirmationKind,
  IssuerOnboardingStatusName
> = {
...
export class IssuerOnboardingService {
  ...
  private async diagnose(issuerId: string, error: unknown): Promise<unknown> {
    if (
      !(error instanceof ArcaRejectionError) ||
      !error.hasCode(ARCA_TOKEN_RELATION_CODE)
    ) {
      return error;
    }
    const issuer = await this.prisma.issuer.findUniqueOrThrow({

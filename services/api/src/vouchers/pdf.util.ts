...
export function renderVoucherPdf(data: VoucherPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    if (data.paymentDueDate) {
      doc
        .fontSize(8)
        .text(`Vencimiento de pago: ${date(data.paymentDueDate)}`, right - 220, 90, {
          width: 220,
          align: "right",
        });
    }
    ...
    if (data.associatedVouchers.length > 0) {
      ...
      for (const voucher of data.associatedVouchers) {
        doc
          .fontSize(8)
          .text(`• ${describeVoucher(voucher.type, voucher.salesPoint, voucher.number)}`, left + 10, y);
        y += 12;
      }
    }
    ...
    doc
      .fontSize(7)
      .fillColor("#666")
      .text("Comprobante autorizado por ARCA. Verificable con el código QR.", left + 140, qrY + 76, {
        width: width - 140,
      });

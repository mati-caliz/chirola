...
describe("comprobantes de exportación (C.3)", () => {
  ...
  it("exige el Incoterm en la exportación de bienes", () => {
    const result = issueExportVoucherSchema.safeParse(input({ exportType: ExportType.GOODS }));
  ...
  it("rechaza un tipo de comprobante que no sea de exportación", () => {
    const result = issueExportVoucherSchema.safeParse(input({ voucherType: VoucherType.FACTURA_A }));

    expect(result.success).toBe(false);
  });

  it("exige el CUIT país del destino", () => {
    expect(issueExportVoucherSchema.safeParse(input({ countryTaxId: "123" })).success).toBe(false);
  });

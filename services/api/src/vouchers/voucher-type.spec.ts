...
describe("tipos de comprobante M (C.1)", () => {
  ...
  it("la B informa IVA a ARCA aunque no lo discrimine en el comprobante", () => {
    expect(reportsIvaToArca(VoucherType.FACTURA_B)).toBe(true);
    expect(reportsIvaToArca(VoucherType.NOTA_CREDITO_B)).toBe(true);
...
describe("inferFiscalCondition con Factura M (C.1)", () => {
  it("detecta al emisor habilitado a M aunque también tenga B", () => {
    expect(inferFiscalCondition([VoucherType.FACTURA_M, VoucherType.FACTURA_B])).toBe(
      FiscalCondition.RESPONSABLE_INSCRIPTO_M,
    );
  });

  it("sigue detectando al responsable inscripto sin M", () => {
    expect(inferFiscalCondition([VoucherType.FACTURA_A, VoucherType.FACTURA_B])).toBe(
      FiscalCondition.RESPONSABLE_INSCRIPTO,
    );
  });

  it("sigue detectando al monotributista", () => {
    expect(inferFiscalCondition([VoucherType.FACTURA_C])).toBe(FiscalCondition.MONOTRIBUTISTA);
  });
});
